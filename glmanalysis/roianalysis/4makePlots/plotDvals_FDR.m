...
for iRoi = 1:numRoi
  ...
  % also do t-test to see if each significantly greater than 0
  [memZeroH memZeroP] = ttest(d(:,1),[],[],'right');
  [detZeroH detZeroP] = ttest(d(:,2),[],[],'right');
  ...
  % save as ill file
  if iRoi == numRoi
    graphName = '/Users/shani/Dropbox/FEF_IPS_paper/figures/in_progress/dPlot_MAIN_FDR_1tail';
    switch whichHRF
      case 'Can'
  ...
  if iRoi == numRoi
    graphName = ['/Users/shani/Dropbox/FEF_IPS_paper/figures/in_progress/dPlot_bySubj_FDR_1tail'];
    switch whichHRF
      case 'Can'
        graphName = [graphName '_cannonHRF'];
      case 'Est'
