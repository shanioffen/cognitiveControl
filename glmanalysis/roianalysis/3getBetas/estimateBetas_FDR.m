function [epochR2 betasROI] = estimateBetas_FDR
...
graphDir = '/Users/shani/NYU/NYUdocuments/HeegerLab/IPSgraphs/';
...
for iExpt = 1:length(exptList)
  ...
  for iSubj = 1:length(subjList)
    ...
    for iRoi = 1:numRoi
      ...
      % need to calculate the TTA for the data and the model together, because
      % are going to subtract the first time point so starts at 0
      [dataTTAcorrect modelTTAcorrect]= getTTA_so(fullRoiTC,glm.model,exptTiming,1); 
      [dataTTAwrong modelTTAwrong] = getTTA_so(fullRoiTC,glm.model, exptTiming,-1);


      % get epoch-based R2
      
      epochR2(iRoi) = getEpochR2(fullRoiTC, glm.model, exptTiming);
      glm.epochR2 = epochR2(iRoi);
      saveGLM(subj,expt,model,ROI,glm,whichHRF)
      ...
      if doplot
        ...
        if iSubj==1 %make special graph for the combined data
          plotNum2 = plotNum2 +1;
          figure(figNum*10) 
          subplot(nR,nC,plotNum2);
          legendHandle(1) = errorbar(-4:2:30, dataTTAcorrect(:,1),dataTTAcorrect(:,2),'r.-');
          legendStr{1} = sprintf('%s %s',subj,expt);
          hold on;
          legendHandle(2) = errorbar(-4:2:30,modelTTAcorrect(:,1),modelTTAcorrect(:,2),'k.-');    
          ...
          xlim([-4 30]);
          ylim([-0.4 0.8])
          title(sprintf('%s %s, r2=%0.2f',expt,ROI,glm.r2))
          cfh2=gcf;
          hline(0,'k');
          %          legend(legendHandle,legendStr)
  ...
  if doplot
    graphName1 = [graphDir expt '_dataPlots_FDR'];
    print(cfh1,'-painters','-dill',graphName1);
    graphName2 = [graphDir expt '_dataAllSubj_FDR'];
    print(cfh2, '-painters','-dill',graphName2);
  end
