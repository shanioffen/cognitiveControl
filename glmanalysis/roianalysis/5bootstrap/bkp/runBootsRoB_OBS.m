...
subjList = {'JG','DS','LM','RS','SO','allSubj'};
  ...
  for jSubj = 1:length(subjList)
    ...
    if(reRun)
      ...
      for iRoi = 1:numRoi
        ...
        tic
          for k = 1:numBoots
            bootstrap(:,iRoi,k) = bootstrapROB_master(subj,expt,roiTC,hrf,hipassfilter,subjRunList);
          end
        toc
      end % run through ROI
      % save 
      saveName = [bootdir 'matFiles/bootstrap_' expt '_' subj];
      save(saveName,'bootstrap');
    else % if don't need to rerun
      load([bootdir 'matFiles/bootstrap_' expt '_' subj]);
    end % if rerun
    ...
    for iRoi = 1:numRoi
      ...
      numGreaterCorr(iExpt,jSubj,iRoi) = sum(bootstrap(2,iRoi,:)>betas(2));
      numGreaterWrong(iExpt,jSubj,iRoi) = sum(bootstrap(2,iRoi,:)>betas(5));
      
    end % going through ROIs
    clear bootstrap % before next subj/expt
  end
end
...
for iSubj = 1:length(subjList)
  subj = subjList{iSubj};
  load([bootdir 'matFiles/bootstrap_memory_' subj]);
  bootsD = bootstrap; clear bootstrap
  load([bootdir 'matFiles/bootstrap_detect_' subj]);  
