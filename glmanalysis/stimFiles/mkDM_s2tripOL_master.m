function mkDM_s2tripOL_master(subj,expt)
...
dataName = [saveDir 'DM_correctTrialS2tripOL_' expt '_' subj];
  ...
  for runInd = 1:numRuns;            
    ...
    for trial = 1:numTrials
      ...
      if corrVal % = 1 for correct
        ...
        % second column: delay
        for time = 1:delayVal
          designMatrix(startInd+time-1,2)=1;
        end 
        ...
        % fifth column: delay
        for time = 1:delayVal
          designMatrix(startInd+time-1,5)=1;
        end 
...
for iRun = 1:totalRuns
  ...
  stimFile = [stimDir 'stimFile_corrTrial_S2tripOL_' num2str(iRun) '.mat'];
