...
function runGLMbyDM(subj,expt,modelNum,HRFnum)
...
% haven't figured out how to do the split yet....

for iModel = modelNum
  model = modelList{iModel};
  modelnik = modelNickname{iModel};
  if strcmp(model,'split'), split = 1; model = 'correctTrialS2plus'; else, split = 0; end
  ...
  params.saveName = ['glm_byDM_' modelnik 'hrf_' whichHRF];
  ...
  % now explicitly setting the DM (will set to params structure after identify scan number)
  if isempty(DM),mrWarnDlg('(runGLMbyDM) No DM found');keyboard,end
  ...
  params.scanNum = 1; % do first scan - could also do all scans, or last scan...
  %params.scanNum = viewGet(view,'nScans',groupNum); % alternatively, take last scan in group
  params.DM{scanNum} = DM; % haven't implemented dif DM for dif scans yet
