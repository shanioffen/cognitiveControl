...
function runGLM_splitOBS(subj,expt)
...
for iModel = modelNum
  ...
  [DMall DModd DMeven] = getDM(subj,expt,model);
  params.DModd = DModd;
  params.DMeven = DMeven;
  if isempty(params.DModd),mrWarnDlg('(runGLM_split) No DM found');keyboard,end
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  % run the GLM for all, odd and even separately:
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

  % then, odd, which are in the 2nd scan in the blur group:
  params.DM = DMall;
  params.scanNum = 1;
  useDefault = 1;
  ...
  [viewAll dAll] = mrLR_FDRoverlay(view,params);
  ...
  disp('(runGLM_split) calling mrLR_FDRoverlay for odd runs')
  [viewOdd dOdd] = mrLR_FDRoverlay(view,params);

  % then, even, which are in the 3rd scan in the blur group:
  params.DM = DMeven;
  params.scanNum = 3;
  useDefault = 1;
  params.scanParams = getEventRelatedParams(view,params,useDefault);
  ...
  disp('(runGLM_split) calling mrLR_FDRoverlay for even runs')
  [viewEven dEven] = mrLR_FDRoverlay(view,params);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [DMall DModd DMeven] = getDM(subj, expt, model)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
