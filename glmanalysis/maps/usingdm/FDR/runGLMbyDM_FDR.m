...
function runGLMbyDM_FDR(subj,expt,modelNum,HRFnum,blur)
...
elseif nargin == 3
  ...
  blur = 0;
elseif nargin ==4
  blur = 0; % set to 1 in order to spatially blur the data, to 0 to leave unblurred;
end
...
for iModel = modelNum
  ...
  params.scanNum = 1:viewGet(view,'nScans',groupNum); 
  params.scanNum = 1;
  params.DM{scanNum} = DM;
  ...
  % run the GLM
  disp('(runGLMbyDM) calling mrLR_FDRoverlay')
