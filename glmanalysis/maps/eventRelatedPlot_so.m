...
s1times = d.stimvol{s1place};
if length(d.stimvol{s2place}) > length(d.stimvol{s1place}) %if model s2 with 2 TRs
  s2times = d.stimvol{s2place}(1:2:length(d.stimvol{s2place}));
else
  s2times = d.stimvol{s2place};
end


numSkips = length(s1times)-length(s2times); 
comp1 = s1times;
comp3 = NaN*ones(size(comp1));
comp3(1:length(s2times)) = s2times;
...
trialStarts = d.stimvol{4};
numTrials = length(d.stimvol{4});
epochTseries = NaN*ones(numTrials,18); 

if numBetas == 6
  s1times = d.stimvol{4};
  if length(d.stimvol{6}) > length(s1times)
    s2times = d.stimvol{6}(1:2:length(d.stimvol{6}));
  else
    s2times = d.stimvol{6};
  end
